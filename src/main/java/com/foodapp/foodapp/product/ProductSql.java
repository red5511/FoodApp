...
public class ProductSql {
    ...
    public static final String BASE_QUERY =
            "SELECT p FROM Product p WHERE p.company.id IN (:" + COMPANY_IDS_PARAM + ") AND :" + STATUS_PARAM + "= p.status";
    public static final String BASE_COUNT_QUERY =
            "SELECT COUNT(p) FROM Product p WHERE p.company.id IN (:" + COMPANY_IDS_PARAM + ")  AND :" + STATUS_PARAM + "= p.status";
